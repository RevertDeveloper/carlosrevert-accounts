...
Reserva: `{"application":"juridia","action":"legal_query","request_id":"UUID"}`. Los callbacks internos deben mandar `X-Application-Slug` y `X-Service-Key` (rotada desde Admin), o el secreto interno de emergencia configurado por entorno. El esquema OpenAPI es la fuente de contrato viva.
